#!/usr/bin/env python3
# Video PLANES DE AHORRO/PIAS con imagenes reales + narracion (16:9 horizontal para redes)
import subprocess, os
from PIL import Image, ImageDraw, ImageFont

IMGS="/home/node/workspace/cfg-seguros/posts/imgs_pias"
BASE="/home/node/workspace/cfg-seguros/posts"
AUDIO=f"{BASE}/pias_audio.mp3"
OUT=f"{BASE}/video_pias_imagenes.mp4"
W,H=1280,720
FPS=24

escenas=[
 ("ahorro_alcancia.jpg",     "HAZ CRECER TU DINERO Y AHORRA IMPUESTOS"),
 ("planificacion.jpg",       "LOS PIAS: PLANES DE AHORRO SISTEMÁTICO"),
 ("dinero_crecer.jpg",       "TU DINERO CRECE AÑO TRAS AÑO"),
 ("grafico_crecimiento.jpg", "RENDIMIENTOS LIBRES DE IMPUESTOS A LOS 5 AÑOS"),
 ("ahorro_monedas.jpg",      "CONSTRUYE TU COLCHÓN PARA EL FUTURO"),
 ("futuro_manos.jpg",        "EL TIEMPO ES TU MEJOR ALIADO"),
 ("grafico_crecimiento.jpg", "EMPIEZA HOY, DISFRUTA MAÑANA"),
 ("planificacion.jpg",       "CFG SEGUROS: PLANEA TU FUTURO FINANCIERO"),
]
...
for i,(img,txt) in enumerate(escenas):
    ...
    slide=f"/tmp/pias_s{i}.png"; im.save(slide)
    seg_i=seg+(0.6 if i==len(escenas)-1 else 0)
    v=f"/tmp/pias_v{i}.mp4"
...
clist="/tmp/pias_lista.txt"
open(clist,"w").write("".join(f"file '{v}'\n" for v in vlist))
subprocess.run(["ffmpeg","-y","-f","concat","-safe","0","-i",clist,"-c:v","libx264","-pix_fmt","yuv420p","/tmp/pias_visual.mp4"],check=True,capture_output=True)
subprocess.run(["ffmpeg","-y","-i","/tmp/pias_visual.mp4","-i",AUDIO,"-c:v","copy","-c:a","aac","-shortest",OUT],check=True,capture_output=True)
for v in vlist: os.remove(v)
for i in range(len(escenas)): os.remove(f"/tmp/pias_s{i}.png")
